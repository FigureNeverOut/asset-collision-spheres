...
from ..geometry.material import material_depth as _material_depth
...
def evaluate_auto_geometry_spheres(mesh, spheres, config, *, seed=20260908, sample_count=12000):
    ...
    if "geometry_policy" in config:
        from .geometry_policy import evaluate_geometry_spheres
        return evaluate_geometry_spheres(mesh, spheres, config, seed=seed, sample_count=sample_count)
    from .joint_selection import is_extended, legacy_config, normalize
    if is_extended(config):
        resolved = normalize(mesh, config)
        config = {**legacy_config(resolved), "sphere_budget": resolved["effective_hard_limit"]}
...
def generate_auto_geometry_spheres(mesh, raw):
    if "geometry_policy" in raw:
        from .geometry_policy import generate_geometry_spheres
        return generate_geometry_spheres(mesh, raw)
    # Preserve the original path byte-for-byte in behaviour for frozen configs.
    # New policy/feature fields opt in to the incremental joint selector.
    from .joint_selection import generate, is_extended
    if is_extended(raw):
        return generate(mesh, raw)
